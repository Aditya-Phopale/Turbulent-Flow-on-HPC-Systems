...
#include "TurbulentVTKStencilKE.hpp"

Stencils::TurbulentVTKStencilKE::TurbulentVTKStencilKE(const Parameters& parameters):
...
void Stencils::TurbulentVTKStencilKE::writeVTKHeader(std::ostream& file) const {
  file << "# vtk DataFile Version 2.0" << std::endl << "NS-EOF" << std::endl << "ASCII" << std::endl << std::endl;
}

void Stencils::TurbulentVTKStencilKE::writePoints(std::ostream& file, RealType simulationTime) const {
...
void Stencils::TurbulentVTKStencilKE::apply(TurbulentFlowFieldKE& flowField, int i, int j) {
  ASSERTION(FieldStencil<TurbulentFlowFieldKE>::parameters_.geometry.dim == 2);
...
void Stencils::TurbulentVTKStencilKE::apply(TurbulentFlowFieldKE& flowField, int i, int j, int k) {
  ASSERTION(FieldStencil<TurbulentFlowFieldKE>::parameters_.geometry.dim == 3);
...
void Stencils::TurbulentVTKStencilKE::openFile(int timeStep, RealType simulationTime) {
  written_ = false;
...
void Stencils::TurbulentVTKStencilKE::write(TurbulentFlowFieldKE& flowField, int timeStep, RealType simulationTime) {
  openFile(timeStep, simulationTime);

  if (FieldStencil<TurbulentFlowFieldKE>::parameters_.geometry.dim == 2) {
    // Write pressure
    ofile_
...
void Stencils::TurbulentVTKStencilKE::closeFile() { ofile_.close(); }
