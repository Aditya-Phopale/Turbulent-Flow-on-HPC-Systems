...
namespace Stencils {
  ...
  class epsBufferFillStencil: public BoundaryStencil<TurbulentFlowFieldKE> {
  private:
  ...
  public:
    epsBufferFillStencil(const Parameters&, std::vector<RealType>&, std::vector<RealType>&, std::vector<RealType>&, std::vector<RealType>&, std::vector<RealType>&, std::vector<RealType>&);
    ~epsBufferFillStencil() override = default;
